...
```
Applying the destination rule has no effect at this point becuase the virtual 
...
traffic to the generic `name` service and **what** workload is going to recieve 
...
You can see in Kiali that the virtual servce combined with the destination 
...
But you can also leverage [TLSRoute](https://istio.io/latest/docs/reference/config/networking/virtual-service/#TLSRoute) 
...
But there is a lot more virtual services can be do for traffic distribution like 
...
workload version. But you can also set traffic policies on a destination rule 
